...
namespace Simplicity.dotNet.Core.Logic {
	...
	public class JniBridgeManager : IJniBridgeManager {
		...
		public ExecutionResult AddPath(string jarFile, ref string result) {
			...
			try {
				if (IsJvmLoaded.Value) {
					...
					retval.LastExceptionIfAny = new Exception("Unable to proceed without JVM being loaded.");
			} catch (Exception e) {
		...
		public ExecutionResult LoadJVM(string JniBridgeLibraryPath, string jniBridgeLibConfigPath) {
				...
				if (IsJvmLoaded.Value) {
					...
					if (found?.Value != null) {
						...
						if ((hProcAddress = Win32Helper.GetProcAddress(jvmHandles["hModuleJniBridge"], JniBridgeMethod.RunMethodInJar)) != IntPtr.Zero) {
							var functor = Marshal.GetDelegateForFunctionPointer(hProcAddress, typeof(Win32Helper.RunMethodInJar)) as Win32Helper.RunMethodInJar;

							retval.IsSuccess = functor(methodInformation.ClassName.Replace(".","/"), methodKey.MethodName,  libraryInformation.JavaMethod.Contains("static"),
													   libraryInformation.JniDescriptor, list, paramType.ToArray(), list.Length,
													   Marshal.GetFunctionPointerForDelegate(returnValueGetter[returnType]), exceptions, returnType) == 0;
